...
jQuery.fn.isInViewportByFrenify2 = function() {
	"use strict";
    var $element 		= jQuery(this);
	...
	var percentage		= viewportBottom - elementTop;

    return [(elementBottom > viewportTop && elementTop < viewportBottom),percentage];
};


var AilandAjax				= AilandAjaxObject;
var AilandBody				= jQuery('body');
var AilandWrapper			= jQuery('.ailand-fn-wrapper');
var AilandVoteWait			= false;
var AilandReactionWait		= false;
var AilandNextPostWait		= false;
var AilandQuickNav			= 0;
var AilandCounterAjaxPost	= 1;
var PoptioEntityMap = {
	'&': '&amp;',
	'<': '&lt;',
	'>': '&gt;',
...
//			this.moreMenu();
			...
			this.navOpener();
			this.projectPopup();
			this.stickyProjectSingle();
			this.carouselProjectSingle();
        },
		
		
		carouselProjectSingle: function(){
			var rtl = false;
			if($("body").hasClass("rtl")){
				rtl = true;
			}
			$('.ailand_fn_project_s_carousel .owl-carousel').each(function(){
				$(this).owlCarousel({
					loop: true,
					autoWidth:true,
					items:4,
		...
		stickyProjectSingle: function(){
			if($('.ailand_fn_project_s_sticky').length){
				AilandBody.css({overflow:'visible'});
			}	
		},
		
		navOpener: function(){
			$('.ailand_fn_offcanvas_opener').off().on('click',function(){
				var e = $(this);
				
				if(e.hasClass('active')){
					e.removeClass('active');
					AilandBody.removeClass('offcanvas-sidebar-opened');
				}else{
					e.addClass('active');
					AilandBody.addClass('offcanvas-sidebar-opened');
				}
				return false;
			});
			
			$('.ailand_fn_vertnav_opener').off().on('click',function(){
				var e = $(this);
				
				if(e.hasClass('active')){
					e.removeClass('active');
					AilandBody.removeClass('vertnav-opened');
				}else{
					e.addClass('active');
					AilandBody.addClass('vertnav-opened');
				}
				return false;
			});
			
			$('.ailand_fn_vertnav_mini_closer').on('click',function(){
				$('.ailand_fn_vertnav_opener').removeClass('active');
				AilandBody.removeClass('vertnav-opened');
				
				return false;
			});
			
			var close = $(".ailand_fn_offcanvas_s_cursor");
			var block = $(".ailand_fn_offcanvas_s_closer");
			
			
			block.on('click',function(){
				$('.ailand_fn_offcanvas_opener').removeClass('active');
				AilandBody.removeClass('offcanvas-sidebar-opened');
				
				return false;
			});
			$('.ailand_fn_offcanvas_sidebar .fn__mobile_closer').on('click',function(){
				$('.ailand_fn_offcanvas_opener').removeClass('active');
				AilandBody.removeClass('offcanvas-sidebar-opened');
		...
		movingLeftMenu: function(){
			var fixedsub 			= $('.ailand_fn_fixedsub');
			var menuBar				= $('.ailand_fn_vertnav');
			var li					= menuBar.find('.ailand_fn_main_nav > li');
			var rightpart			= $('.ailand_fn_content');
			...
			li.on('mouseenter', function(){
				...
				var left			= menuBar.offset().left;

				fixedsub.css({top:asd,left:left+menuBar.outerWidth(true,true)});
				abc();
			});
			function abc(){
		...
		movingSearch: function(){
			var close = $(".ailand_fn_search_cursor");
			var block = $(".ailand_fn_search_closer");
		...
		subscribeFocus: function(){
			$('.footer_subscribe_form input[type=email], .footer_subscribe_form div.wpforms-container-full .wpforms-form input[type=email]').on('focusout',function(){
				$(this).closest('.footer_subscribe_form').removeClass('focused');
			}).on('focus',function(){
				$(this).closest('.footer_subscribe_form').addClass('focused');
			});
		},
		...
		copyToClibpoard: function(){
			$('.frenify__copy_link').off().on('click',function(){
				...
				var box = $('<div class="frenify__copy_dialog">'+AilandAjax.copiedText+'</div>');
				$("body").append(box);
				setTimeout(function(){
					box.remove();
				},500);
		...
		sticky: function(){
			$('.ailand_fn_leftsidebar, .ailand_fn_rightsidebar').theiaStickySidebar({
				'containerSelector':'',
		...
		topQuickNav: function(){
			$('.fn__blog_anchor').mousedown(AilandInit.handle_mousedown);
			$('.fn__blog_anchor .closer').off().on('click',function(){
				$('.fn__blog_anchor').removeClass('active').css({left: '100%'});
				AilandBody.removeClass('blog-anchor-active');
				AilandQuickNav = 0;
				return false;
			});
			
			var stickyH = 0;
			if($('.ailand_fn_stickynav').length){
				stickyH = $('.ailand_fn_stickynav').outerHeight();
			}
			
			$('.fn__blog_anchor .ba_item').off().on('click',function(){
				var id = $(this).data('id');
				$([document.documentElement, document.body]).animate({
					scrollTop: $('.ailand_fn_blog_single[data-post-id="'+id+'"]').offset().top - 80 - stickyH
				}, 600);
			});
		},
		...
		runPlayer: function(){
			var parent		= $('.ailand_fn_main_audio');
...
//					features: ['playpause','skipback','jumpforward','progress','current','duration','tracks','volume'],
		...
		newPlayer: function(){
			var audiobox			= $('.ailand_fn_main_audio');
			var playButton			= $('.ailand_fn_audio_button');
			var boxCloser 			= audiobox.find('.closer');
			var audioOpener 		= $('.ailand_fn_audio_opener');
			...
			boxCloser.off().on('click', function(){
				...
				if(audiobox.hasClass('fn_play')){
					...
					AilandBody.removeClass('music-play');
			...
			playOfAudiobox.off().on('click', function(){
				if(audiobox.find('audio,video').length){
					if(audiobox.hasClass('fn_pause')){
						...
						AilandBody.addClass('music-play');
			...
			playButton.off().on('click',function(){
				...
				if(button.closest('.fn__mp3_item').length){
					...
					if(mp3Item.hasClass('fn_pause')){
						...
						audioOpener.removeClass('opened');
						AilandBody.addClass('music-play');
						$('.ailand_fn_main_audio .audio_player').html('<audio controls><source src="'+button.attr('data-mp3')+'" type="audio/mpeg"></audio>');
						self.runPlayer();
						setTimeout(function(){
		...
		stickyTopBar: function(){
			var stickyHeader = $('.ailand_fn_stickynav');
		...
		stickyHeader: function(){
			...
			if(stickyHeader.length){
				...
				if(scrollTop > $('.ailand_fn_header').outerHeight() + 50){
					AilandBody.addClass('sticky-active');
				}else{
					AilandBody.removeClass('sticky-active');
				}
			}
		},
		
		transformReading: function(){
			var stickyHeader = $('.ailand_fn_stickynav.ajax_enable');
			if(stickyHeader.length && AilandBody.hasClass('single-post')){
				var lastScrollTop = 0;
		...
		nowReading: function(){
			...
			var progress 	= $('.ailand_fn_stickynav .progress');
			$(window).on('resize scroll', function() {
				var bs 		= $('.ailand_fn_blog_single');
				bs.each(function(){
					var e 	= $(this);
		...
		getSidePopupPost: function(){
			...
			if(sidepp.length){
				$(document).scroll(function() {
					var footerHeight =  $('#ailand_fn_footer').length > 1 ?  $('#ailand_fn_footer').outerHeight() : 0;
		...
		ajaxNextPost: function(){
			var singlePost = $('.ailand_fn_singleajax');
			if($('.ailand_fn_singleajax').length && !AilandNextPostWait){
				$(document).scroll(function() {
					if ((window.innerHeight + window.scrollY) >= document.body.offsetHeight - $('#ailand_fn_footer').outerHeight()) {
						if(!AilandNextPostWait){
							var single			= $('.ailand_fn_blog_single');
							var ID 				= single.last().data('get-post-id');
							if(ID === ''){
								AilandNextPostWait = true;
								AilandBody.addClass('no-prev-post');
								return false;
							}
							AilandBody.addClass('prev-post-loading');
							AilandNextPostWait 	= true;
							var requestData 	= {
								action: 'ailand_fn_get_prev_post', 
								ID: ID,
								security: AilandAjax.nonce
							};

							$.ajax({
								type: 'POST',
								url: AilandAjax.ajax_url,
								cache: false,
								data: requestData,
								success: function(data) {
									...
									if(AilandQuickNav === 1){
										...
										AilandBody.addClass('blog-anchor-active');
									}
									var fnQueriedObj 	= $.parseJSON(data); //get the data object
									singlePost.append(fnQueriedObj.output);
									AilandInit.init();
									AilandNextPostWait = false;
									AilandBody.removeClass('prev-post-loading');
									
									
									AilandCounterAjaxPost++;
									var appendedElement = singlePost.children().last();
									var count = AilandCounterAjaxPost < 10 ? '0' + AilandCounterAjaxPost : AilandCounterAjaxPost;
		...
		featuredPostsWidget: function(){
			$('.ailand_fn_widget_featured .swiper-container').each(function(){
				var element				= $(this);
				// Main Slider
				var mainSliderOptions 	= {
					loop: true,
		...
		footerPosts: function(){
			$('.ailand_fn_vertical_slider').each(function(){
				var e = $(this);
				if(e.hasClass('ready')){
					return false;
				}
				...
				AilandInit.vs_changeslide(1,e);
				AilandInit.vs_start(vs,e);
			});
		},
		...
		vs_start: function(vs,element){
			...
			time = setInterval(function(){
				...
				AilandInit.vs_changeslide(index,element);
			}, timeout);
		},
		...
		voteOpener: function(){
			$('.ailand_fn_votes').off().on('click',function(){
				var e = $(this);
			...
			$('.ailand_fn_vote_up').off().on('click',function(e){
				e.preventDefault();
				var element = $(this);
				if(element.closest('.ailand_fn_votes').hasClass('up_action')){
					return false;
				}
				AilandInit.vote(element,'up');
				return false;
			});
			
			
			$('.ailand_fn_vote_down').off().on('click',function(e){
				e.preventDefault();
				var element = $(this);
				if(element.closest('.ailand_fn_votes').hasClass('down_action')){
					return false;
				}
				AilandInit.vote(element,'down');
		...
		vote: function(element,action){
			if(AilandVoteWait === true) {return false;}
			var parent			= element.closest('.ailand_fn_votes');
			parent.addClass('loading');
			AilandVoteWait 		= true;
			var ID 				= parent.data('id');
			var requestData 	= {
				action: 'ailand_fn_vote', 
				ID: ID,
				voteAction: action,
				security: AilandAjax.nonce
			};
		...
		reaction: function(){
			$('.ailand_fn_reaction_btn').off().on('click',function(){
				var element = $(this);
				if(AilandReactionWait === true) {return false;}
				var parent			= element.closest('.ailand_fn_reactions');
				parent.addClass('loading');
				AilandReactionWait	= true;
				var ID 				= element.data('id');
				var requestData 	= {
					action: 'ailand_fn_reactions', 
					ID: parseInt(ID),
					ajax_action: AilandInit.escapeHTML(element.data('action')),
					security: AilandAjax.nonce
				};
				...
				$.ajax({
					...
					success: function(data) {
						...
						element = $('.ailand_fn_reaction_btn[data-id="'+ID+'"][data-action="'+newReaction+'"]');
		...
		commentOpener: function(){
			$('.ailand_fn_comments .comment_opener a').off().on('click',function(){
				var element = $(this);
				var parent = element.closest('.ailand_fn_comments');
		...
		sidebarOpener: function(){
			var rightbar 	= $('.ailand_fn_rightbar');
			var trigger		= $('.ailand_fn_stickynav .right__trigger a,#ailand_fn_header .right__trigger a');
			trigger.off().on('click',function(){
			...
			$('.ailand_fn_rightbar .bar_closer a,.ailand_fn_rightbar .bar_extra_closer').off().on('click',function(){
				trigger.removeClass('active');
				rightbar.removeClass('active');
				return false;
			});
		...
		moreMenu: function(){
			$('.ailand_fn_nav').each(function(){
				...
				if(nav.parent().hasClass('bottom_fixer')){
					menu = $('.ailand_fn_header .bottom_fixer');
					padding = 120;
				}
				nav.find('.ailand_fn_main_nav > li').each(function(){
					var e = $(this);
		...
		reversedMenu: function(){
			$('.ailand_fn_main_nav ul').each(function(){
				var e = $(this),
					w = e.offset().left + 240,
					W = $('body').width();
				if(w>W){
		...
		seachSomething: function(){
			var searchOpener 	= $('.ailand_fn_search_opener,.ailand_fn_mobnav .mobsearch_opener, .icon_bar__search a');
			var searchbox 		= $('.ailand_fn_searchbox');
			...
			searchOpener.off().on('click',function(){
				if(AilandBody.hasClass('search-active')){
					AilandBody.removeClass('search-active');
				}else{
					AilandBody.addClass('search-active');
					input.val('');
					setTimeout(function(){
						input[0].focus();
					},100);
			...
			$('.ailand_fn_search_closer').off().on('click',function(){
				AilandBody.removeClass('search-active');
				resultList.html('');
			...
			$('.ailand_fn_searchbox .fn__mobile_closer').off().on('click',function(){
				AilandBody.removeClass('search-active');
				resultList.html('');
				infoBox.html('<p>'+infoBox.data('info')+'</p>');
				return false;
			...
			input.on("keypress", function(event) {
				if (event.key === "Enter") {
					...
					$('.ailand_fn_searchbox form input[type="submit"]').trigger('click');
				}
			});
		...
		totopScroll: function(){
			var minSpeed 		= 500;
			var maxSpeed		= 1500;
			$(".ailand_fn_totop").off().on('click', function(e) {
				e.preventDefault();
		...
		ready: function(){
			$('.ailand_fn_walletbox, .ailand_fn_wallet_closer, .ailand_fn_leftnav, .ailand_fn_leftnav_closer').removeClass('ready');
		},
		
		countdown: function(){
			$('.ailand_fn_countdown').each(function(){
				var e = $(this),
					t = e.data('type');
				if(t === 'due_date'){
				...
				if(e.hasClass('boxed')){
					e.after('<div class="ailand_fn_boxed_countdown"><span class="left_wing"></span><span class="right_wing"></span><ul><li class="days"><div class="item"><div class="count"><h3 class="fn__title">0</h3></div><span>'+e.data('text-days')+'</span></div></li><li class="hours"><div class="item"><div class="count"><h3 class="fn__title">0</h3></div><span>'+e.data('text-hours')+'</span></div></li><li class="minutes"><div class="item"><div class="count"><h3 class="fn__title">0</h3></div><span>'+e.data('text-minutes')+'</span></div></li><li class="seconds"><div class="item"><div class="count"><h3 class="fn__title">0</h3></div><span>'+e.data('text-seconds')+'</span></div></li></ul></div>');
					var p = e.parent().find('.ailand_fn_boxed_countdown');
					e.remove();
				}
				if(t === 'due_date'){
		...
		minHeightPages: function(){
			...
			var footer 			= $('#ailand_fn_footer');
			var header 			= $('#ailand_fn_header');
			var footerHeight	= 0;
			var headerHeight	= 0;
			...
			if (window.matchMedia('(max-width: 1040px)').matches) {
				mobileheaderHeight = $('.ailand_fn_mobnav .mob_top').height();
			}
			if(header.length){
			...
			$('.ailand_fn_page_ajax').css({minHeight: ($(window).height() - adminBarHeight - footerHeight - headerHeight - mobileheaderHeight ) + 'px'});
		},
		
		fixAdminBar: function(){
			if(AilandBody.hasClass('admin-bar')){
				$('html').addClass('frenify-html');
			}
			if($('.ailand_fn_author_info .info_img img').length){
				$('.ailand_fn_author_info .info_in').css({marginTop: 0});
			}
		},
		
		
		preloader: function(){
			$('.ailand_fn_preloader').addClass('ready');
		},
		...
		widgetTitle: function(){
			$('.wp-block-group__inner-container > h1,.wp-block-group__inner-container > h2,.wp-block-group__inner-container > h3,.wp-block-group__inner-container > h4,.wp-block-group__inner-container > h5,.wp-block-group__inner-container > h6').each(function(){
				var e = $(this);
				e.after('<div class="wid-title"><span class="text">'+e.text()+'</span><span class="icon"><span></span></span></div>');
				e.remove();
			});
		},
		...
		blog_info: function(){
			if($('.blog_info').height() === 0){
				$('.ailand_fn_comment').addClass('margin-no-top');
			}
		...
		projectPopup: function(){
			$('.ailand_popup_gallery').each(function() { // the containers for all your galleries
				$(this).magnificPopup({
			...
			$('.ailand_popup_youtube, .ailand_popup_vimeo').each(function() { // the containers for all your galleries
				$(this).magnificPopup({
					disableOn: 700,
					type: 'iframe',
					mainClass: 'mfp-fade',
			...
			$('.ailand_popup_soundcloude').each(function(){
				$(this).magnificPopup({
					type : 'image',
					gallery: {
						enabled: true, 
		...
		portfolioContentHeight: function(){
			var portfolio = $('.ailand_fn_portfolio_page .portfolio_content');
		...
		url_fixer: function(){
			$('a[href*="fn_ex_link"]').each(function(){
				var oldUrl 	= $(this).attr('href'),
					...
					newUrl  = AilandAjax.siteurl + "/" + array[1];
				$(this).attr('href', newUrl);
			});
			if($('.ailand-fn-protected').length){
				$('.ailand_fn_pagein').css({paddingTop: 0});
			}
		},
			...
			if (myCursor.length) {
				if ($("body").length) {
					...
					var n, i = 0,W = 0,intro = 0,
						o = !1;
					if($('.ailand_fn_intro').length){intro=1;}
					
					var buttons = ".fn_cs_testimonials .fn_nav div, .fn_cs_images_cards li,.fn__blog_anchor .ba_item,.modal_ux_closer, .ailand_fn_nav .trigger,.ailand_fn_header .trigger,.fn_cs_intro_testimonials .prev, .fn_cs_intro_testimonials .next, .fn_cs_swiper_nav_next, .fn_cs_swiper_nav_prev, .fn_dots, .swiper-button-prev, .swiper-button-next, .fn_cs_accordion .acc_head, .ailand_fn_popupshare .share_closer, .ailand_fn_header .fn_finder, .ailand_fn_header .fn_trigger, a, input[type='submit'], .cursor-link, button";
					...
					// slider mouse enter
					AilandBody.on('mouseenter', sliders, function(){
					...
					// slider mouse hold
					AilandBody.on('mousedown', sliders, function(){
		...
		widget__archives: function(){
			$('.widget_archive li').each(function(){
				...
				AilandBody.append('<div class="frenify_hidden_item"></div>');
		...
		prev_next_posts: function(){
			if($('.ailand_fn_siblings')){
				$(document).keyup(function(e) {
					if(e.key.toLowerCase() === 'p') {
						var a = $('.ailand_fn_siblings').find('a.previous_project_link');
						if(a.length){
					...
					if(e.key.toLowerCase() === 'n') {
						var b = $('.ailand_fn_siblings').find('a.next_project_link');
						if(b.length){
		...
		submenu__Mobile: function(){
			var nav 						= $('ul.vert_menu_list, .widget_nav_menu ul.menu, .ailand_fn_mobnav .mob_bot .mobile_menu');
			var mobileAutoCollapse			= AilandWrapper.data('mobile-autocollapse');
			nav.each(function(){
				$(this).find('a').off().on('click', function(e){
					var element 			= $(this);
					var parentItem			= element.parent('li');
					...
					if(mobileAutoCollapse === 'enable'){
						if(nav.parent().parent().hasClass('opened')){
							...
							$('.ailand_fn_mobilemenu_wrap .hamburger').removeClass('is-active');
						}
					}
				});
			});
		...
		hamburgerOpener__Mobile: function(){
			var hamburger		= $('.ailand_fn_mobilemenu_wrap .hamburger');
			hamburger.off().on('click',function(){
				var element 	= $(this);
				var menupart	= $('.ailand_fn_mobilemenu_wrap .mobilemenu');
	...
	$(window).on('resize',function(e){
		e.preventDefault();
		AilandInit.isotopeMasonry();
		AilandInit.minHeightPages();
		AilandInit.moreMenu();
		AilandInit.quickNavResize();
		AilandInit.single_post__gallery();
		AilandInit.blogPageFeatured();
	});
	
	// scroll functions
	$(window).on('scroll', function(e) {
		e.preventDefault();
		AilandInit.totopWinScroll();
		AilandInit.stickyHeader();
    });
	
	// load functions
	$(window).on('load', function(e) {
		e.preventDefault();
		AilandInit.preloader();
		AilandInit.isotopeMasonry();
		setTimeout(function(){
			AilandInit.isotopeMasonry();
			AilandInit.single_post__gallery();
	...
	$( window ).on( 'elementor/frontend/init', AilandInit.rippleEffect );
